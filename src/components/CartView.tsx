...
export function CartView() {
  ...
  const handleWhatsAppOrder = () => {
    ...
    let message = `Hello, I'd like to order the following from Girls Store:\n\n`;
    
    items.forEach((item, index) => {
      message += `${index + 1}. ${item.name} (Qty: ${item.quantity}) - Price: $${(item.price * item.quantity).toFixed(2)}\n`;
    });
